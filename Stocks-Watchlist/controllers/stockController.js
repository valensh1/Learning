...
stockRouter.post('/', async (req, res) => {
    try {
        ...
        res
          .status(200)
          .json(newStockPost) // return json of our newly created blog so we can send to rest of app
    } catch (error) {
        res
        .status(400)
        .json(error)
...
stockRouter.put('/:id', async (req, res) => {
    try{
        const foundStocks = await (await Stock.findByIdAndUpdate(req.params.id, req.body, {new: true})) // find req.params.id and replaces with req.body and gives us document after its changed is reflected by the new:true
        res
          .status(200)
          .json(foundStocks)
    } catch (error){
